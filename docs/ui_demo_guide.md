...
> 适用版本：rag-agent 最终交付（含 PostgreSQL 混合持久化）  
...
3. 结束语：完整 API 与 Docker 部署见 README；LoRA 微调**本阶段未接入**，生成仍走 DashScope `qwen-plus`。
...
可与 [delivery_checklist.md](delivery_checklist.md) 第 12–13 节验收项一并提交。
